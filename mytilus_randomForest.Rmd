...
#Kombinieren von Presence und Absence Punkten und schreiben als 1 für presence und 2 für absence
myt.env2$PA2 <- 1
myt.absence$PA2 <- 0
library(maptools)
...
gridded(sed.pixel)
library(gstat)
depth.pixel_on_sed.pixel <-idw(formula=V3~1,locations=depth.pixel,newdata=sed.pixel)
sed.raster <- raster(sed.pixel,layer=3)
#sed.raster[] <- 1:ncell(sed.raster)
depth.raster <- raster(depth.pixel,layer=3)

#depth.raster[] <- 1:ncell(depth.raster)
#extent(depth.raster)<-c(-3.89722,11.51189,50.91667,61)
extent(depth.raster)<-c(xmin(sed.raster),xmax(sed.raster),ymin(sed.raster),ymax(sed.raster))
res(sed.raster) <- c(xres(depth.raster),yres(depth.raster))
sed.depth.merge.raster <- merge(sed.raster,depth.raster)
...
>>>>>>> df2c81bf4632e093c6cdf4847f5d8721dffcbf9c
```
