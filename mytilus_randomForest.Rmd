...
Die Mytilus- und die Korngrößen-Daten werden nun zusammengeführt und auf den gemeinsamen Bereich zugeschnitten. Damit reduziert sich die Anzahl der Punkte auf 631.

```{r}
library(crecs)
myt.env<-get.environ(points=myt,env=stack(sed.pixel))
myt.env <- myt.env[!is.na(myt.env$val),]
summary(myt.env)
```

Es sollen nun 631 Hintergrundwerte, an denen die Miesmuschel nicht vorkommt, zufällig generiert werden (random-absence). Dies geschieht mit der Funktion randomPoints, die eine Matrix mit x und y Werten erzeugt

```{r}
library(dismo)
random.absence <- randomPoints(sed.pixel,n=631,myt)
```
...
Die Presence-Punktdaten und die generierten Absence-Punktdaten werden jetzt mit einander verbunden.
