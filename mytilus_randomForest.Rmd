...
library(rworldxtra)
data(countriesHigh)
shape_countries <- list(list("sp.polygons",countriesHigh,fill="grey")
                        #list("polygonsLabel",countriesHigh,label=countriesHigh$NAME)
                        )
```
...
spplot(Predictor,"prediction1",main=list("Occurence probability",cex=1),
...
grid.text("depth",x=unit(0.99,"npc"),y=unit(0.50,"npc"),rot=-90,check.overlap = TRUE)
```
...
#North <- list("SpatialPolygonsRescale",layout.north.arrow(type=1),offset=c(000000,520000))
#Um Staedte in der Karte abzubilden wird der Datensatz `world.cities` aus dem Paket `maps` verwendet

library(maptools) #wird benötigt für sp.pointLabel

plot_mytilusoccurence_withdepthmgs <- spplot(Predictor,"predictionWithDepth",main=list("Occurence probability",cex=1),
       col.regions = brewer.pal(9,"OrRd"),
...
mgsPA.bind$PA2 <- as.factor(mgsPA.bind$PA2)
...
spplot(sed.pixel,"prediction",
       main=list("Occurence probability",cex=1),
       col.regions = brewer.pal(9,"OrRd"),
       at=seq(from=0,to=1,by=1/9),
       scales=list(draw=TRUE),
       ylim=c(50.924999,60.99167),xlim=c(-3.882318,11.49699),
...
depth.pixel <- SpatialPixelsDataFrame(depth.etopo,depth.etopo.df)
  #Aus diesem Datensatz wollen wir nur die Werte <0 haben und keine Minuszeichen
depth.pixel$V3 <- depth.pixel$V3*(-1)
depth.pixel <- subset(depth.pixel,depth.pixel$V3>0)
...
depth.pixel_on_sed.pixel <- subset(depth.pixel_on_sed.pixel,depth.pixel_on_sed.pixel$var1.pred>0) #es sollen nur positive Tiefenwerte genommen werden
...
spplot(intersect.part2,"prediction",main=list("Occurence probability",cex=1),
       col.regions = brewer.pal(9,"OrRd"),
       at=seq(from=0,to=1,by=1/9),
       scales=list(draw=TRUE),
       ylim=c(51.062591,55.971701),xlim=c(-2.250885,10.5),
...
spplot(intersect.part2.withdepth,"prediction",main=list("Occurence probability",cex=1),
       col.regions=brewer.pal(9,"OrRd"),at=seq(from=0,to=1,by=1/9),
       scales=list(draw=TRUE), #xlim=c(intersect.part2.withdepth@bbox["x","min"]-3,
                                  #    intersect.part2.withdepth@bbox["x","max"]+3),
       sp.layout=list(
...
```{r Histogramme}
par(mfrow=c(4,3))
hist_phi_absence <- hist(sed.absence$val,main="phi on absence points")
hist_phi_presence <- hist(myt.env$val,main="phi on presence points")
hist_phi_all <- hist(depth.pixel_on_sed.pixel$val,main="phi on whole area")

hist_lat_absence <- hist(sed.absence$lat,main="Latitude on absence points")
hist_lat_presence <- hist(myt.env$lat,main="Latitude on presence points")
hist_lat_all <- hist(as.data.frame(depth.pixel_on_sed.pixel)$lat,main="Latitude on whole area")

hist_lon_absence <- hist(sed.absence$lon,main="Longitude on absence points")
hist_lon_presence <- hist(myt.env$lon,main="Longitude on presence points")
hist_lon_all <- hist(as.data.frame(depth.pixel_on_sed.pixel)$lon,main="Longitude on whole area")
...
par(mfrow=c(1,3))
depth.pixel$log2depth <- log2(depth.pixel$V3)
logdepth_on_absencepoints <- get.environ(sed.absence,stack(depth.pixel))
hist_logdepth_absence <- hist(logdepth_on_absencepoints$log2depth,
                              main="log2 Depth on absence points",breaks=10,xlim=c(0,10)) 
logdepth_on_presencepoints <- get.environ(myt.env,stack(depth.pixel))
hist_logdepth_presence <- hist(logdepth_on_presencepoints$log2depth,
...
which(depth.pixel_on_sed.pixel$logdepth<0)
...
spplot(PA_points,"PA",sp.layout=list("sp.polygons",countriesHigh,fill="grey"),cex=0.5,col.regions=c("black","red"))
