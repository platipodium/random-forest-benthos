...
spplot(Predictor,"depth",main=list("Bathymetry",cex=1),
       ...
       scales=list(draw=TRUE),
       sp.layout=list(list("sp.polygons",countriesHigh,fill="grey"),
...
library(maps)
cities_subset <- subset(world.cities,world.cities$long>=-2.250885&world.cities$long<=8.714282
                        &world.cities$lat>=51.062591&world.cities$lat<=55.971701
...
plot_mytilusoccurence_withdepthmgs <- spplot(Predictor,"predictionWithDepth",main=list("Occurence probability",cex=1),
       ...
       sp.layout=list(
         ...
         list("panel.points",cities_subset$long,cities_subset$lat,pch=20),
...
grid.text("WGS84",x=unit(0.2,"npc"),y=unit(0.2,"npc"))
...
spplot(mgsDepthPA,"V3",cex=0.5,
       main=list("Median grain size",cex=1),
       sp.layout=list("sp.polygons",countriesHigh,fill="grey"),
       col.regions=terrain.colors(6)[1:5],
       scales=list(draw=TRUE)
       )
```
